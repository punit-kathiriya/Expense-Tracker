...
const handleAddPrice = () => {
  ...
  const handleAddCar = (event) => {
    ...
    if (
      ...
      Model &&
      Is_Electric &&
      ((Is_Electric === "1" && Battery) || (Is_Electric === "0" && Tank))
    ) {
      fetch("http://localhost:4000/api/cars", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        ...
        .then((data) => {
          setCars([
            ...cars,
            { ID: data.id, Manufacturer, Model, Tank, Battery, Is_Electric },
          ]);
        })
        .catch(console.error);
    }
  ...
  return (
    <div>
      {currentUser && (
        <AppNav currentUser={currentUser} onSignOut={handleSignOut} />
      )}
      {location.pathname === "/signup" && <SignUp onSubmit={handleAddUser} />}
      {location.pathname === "/signin" && (
        <SignIn currentUser={currentUser} onSubmit={handleCheckUser} />
      )}
      {location.pathname === "/cars/add" && <AddCar onSubmit={handleAddCar} />}
      {location.pathname === "/main" && <Main onSubmit={handleAddPrice} />}
    </div>
  );
}
...
function APIMain(props) {
  return <API {...props}>{(onSubmit) => <Main onSubmit={onSubmit} />}</API>;
}

reportWebVitals();

export { API, APISignUp, APISignIn, APIAddCar, APIMain };
